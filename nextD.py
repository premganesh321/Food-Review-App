...
def show_menu():
    ...
    if current_day=='Monday' :
     cursor.execute("SELECT Dinner FROM menu_monday")
     rows = cursor.fetchall()
     for row in rows:
         menu_list.insert("end", row[0])

    elif current_day=='Tuesday' :
     cursor.execute("SELECT Dinner FROM menu_tuesday")
     rows = cursor.fetchall()
     for row in rows:
         menu_list.insert("end", row[0])

    elif current_day=='Wednesday' :
     cursor.execute("SELECT Dinner FROM menu_wednesday")
     rows = cursor.fetchall()
     for row in rows:
         menu_list.insert("end", row[0])

    elif current_day=='Thursday' :
     cursor.execute("SELECT Dinner FROM menu_thursday")
     rows = cursor.fetchall()
     for row in rows:
         menu_list.insert("end", row[0])

    elif current_day=='Friday' :
     cursor.execute("SELECT Dinner FROM menu_friday")
     rows = cursor.fetchall()
     for row in rows:
         menu_list.insert("end", row[0])

    elif current_day=='Saturday' :
     cursor.execute("SELECT Dinner FROM menu_saturday")
     rows = cursor.fetchall()
     for row in rows:
         menu_list.insert("end", row[0])

    else :
     cursor.execute("SELECT Dinner FROM menu_sunday")
     rows = cursor.fetchall()
     for row in rows:
         menu_list.insert("end", row[0])
