...
def show_menu():
    ...
    if current_day=='Monday' :
     cursor.execute("SELECT Breakfast FROM menu_monday")
     rows = cursor.fetchall()
     for row in rows:
         menu_list.insert("end", row[0])

    elif current_day=='Tuesday' :
     cursor.execute("SELECT Breakfast FROM menu_tuesday")
     rows = cursor.fetchall()
     for row in rows:
         menu_list.insert("end", row[0])

    elif current_day=='Wednesday' :
     cursor.execute("SELECT Breakfast FROM menu_wednesday")
     rows = cursor.fetchall()
     for row in rows:
         menu_list.insert("end", row[0])

    elif current_day=='Thursday' :
     cursor.execute("SELECT Breakfast FROM menu_thursday")
     rows = cursor.fetchall()
     for row in rows:
         menu_list.insert("end", row[0])

    elif current_day=='Friday' :
     cursor.execute("SELECT Breakfast FROM menu_friday")
     rows = cursor.fetchall()
     for row in rows:
         menu_list.insert("end", row[0])

    elif current_day=='Saturday' :
     cursor.execute("SELECT Breakfast FROM menu_saturday")
     rows = cursor.fetchall()
     for row in rows:
         menu_list.insert("end", row[0])

    else :
     cursor.execute("SELECT Breakfast FROM menu_sunday")
     rows = cursor.fetchall()
     for row in rows:
         menu_list.insert("end", row[0])
